...
$\underbrace{p_0\times p_0}_{\gamma} \times \Bern(p)$ and $p_1 \times \Bern(q)$. By independent conditions, we can decompose the
...
Under certain parameter configurations specified in \eqref{eq:oneC},
...
We mention that \eqref{eq:oneC} is satisfied when the difference between $a$ 
and $b$ is large while the distribution $p_0$ is near to $p_1$. This makes the divergence
smaller compared with the magnitude of $()$
...
\section{Analysis of Error Probability}\label{s:proof}
...
=&  \exp(-\log n(\gamma D_{2/3}(p_1 || p_0) \\
