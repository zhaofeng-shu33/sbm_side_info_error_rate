...
\begin{proof}[Proof of Lemma \ref{lem:single_lower}] 
	...
	&\quad\cdot P(\sum_{i=1}^{n-2} (z'_i - z_i) \geq 0)\\
...
\begin{lemma}\label{lem:p0p1}
	...
	\begin{equation}\label{eq:32}
		...
		\leq (\sum_{x\in \mathcal{X}} \sqrt{p_0(x) p_1(x)})^2
	\end{equation}
\end{lemma}
\begin{proof}
...
$$
for $2\leq k \leq \frac{n}{\sqrt{\log n}}$, we can obtain
\begin{align*}
P_e &\leq \frac{1}{4}n^{-\mu + o(1)} +(1+o(1)) \sum_{k=2}^{\frac{n}{\sqrt{\log n}}} \exp(k((-\mu + o(1)) \log n )) \\
& =\frac{1}{4}n^{-\mu + o(1)}+(1+o(1)) \frac{n^{-\mu + o(1)}}{1-n^{-\mu + o(1)}} = (\frac{1}{4}+o(1))n^{-\mu + o(1)}
\end{align*}
...
$P(\cup_{1\leq i < j\leq n} A_{ij})$ by
\eqref{eq:bonf},
...
Since $|\{1\leq i < j\leq n, y^*_i=-y^*_j\}|=(n/2)^2$, the term $\sum_{1\leq i < j\leq n} P(A_{ij})$ is of order $\frac{1}{4}n^{-\mu+o(1)}$.
...
Then we conclude that $P_e=P(F) \geq P(\cup_{1\leq i < j\leq n} A_{ij}) \geq
